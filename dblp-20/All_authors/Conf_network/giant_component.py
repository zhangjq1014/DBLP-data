...
for key in conf_network.keys():
    ...
    years = [int(x) for x in conf_year_authors[key].keys()]
    y = max(years) - min(years)
    if y < 5:
        continue
    name_k_list.append([key, [len(largest_cc) / len(network)] + [len(conf_network[key])], conf_field_level[key][1]])


    colors = ['b']
    if key in ['NSDI', 'ICNP']:
        fout = open(key + '.csv', 'w')
        for au1, au2 in network.edges():
            print (au1, au2, file = fout, sep = ';')
            print (au2, au1, file = fout, sep = ';')
        fout.close()
        ...
        for i, item in enumerate(components):
            ...
            #nx.draw_kamada_kawai(network.subgraph(node_list_temp), node_color = node_color)
    '''
        
        print (nx.is_connected(network.subgraph(node_list)))
    years = [int(x) for x in conf_year_authors[key].keys()]
    y = max(years) - min(years)
    if y < 5:
        continue
        print (nx.number_connected_components(network.subgraph(node_list)))
        input()
        sub_graph = network.subgraph(node_list)
        pos =  nx.spring_layout(sub_graph)
        nx.draw_random(sub_graph, node_color = color_list, node_size = 30)
        plt.savefig('ASAP_spring.png')
        plt.clf()
...
plt.savefig('gaint_component.png', dpi = 400)
